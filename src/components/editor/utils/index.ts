...
export const utils = {
  ...
  getDotPosition(): string {
    return utils.isMobile()
      ? CONSTANTS.DOT_POSITION.MOBILE
      : CONSTANTS.DOT_POSITION.DESKTOP;
  },

  getDotSize(): number {
    return utils.isMobile()
      ? CONSTANTS.DOT_SIZE.MOBILE
      : CONSTANTS.DOT_SIZE.DESKTOP;
  },
  ...
  removeResizeElements(container: HTMLElement): void {
    // Remove all resize handle dots by targeting the data attribute
    const dots = container.querySelectorAll('[data-resize-dot="true"]');
    dots.forEach((dot) => {
